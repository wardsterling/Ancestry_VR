...
if(!archive){for(const file of ['archive-private-details.json','assets/report-portraits','source-documents','source-pages','wall-catalog.json','dist/client/archive-data.json','dist/archive-private-details.json','dist/assets/report-portraits','dist/source-documents']){let found=false;try{await access(path.join(root,file));found=true;}catch{}if(found)throw Error('Private source media is present. Use a clean code-only checkout for a public build.');}}
